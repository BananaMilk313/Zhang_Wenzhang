function [subLiDAR,scan,velmsg,Odom,PubVel,OdomPos,SubOdom,rate,euler, ...
    ...
    =navigation( ...
    end_x,end_y,stop_tag, ...%End point
...
break;
    ...
    if Min<0.5 % Start turning when the distance is less than 0.5
        ...
        if abs(angleDifference) > deg2rad(30-10)  % When the angle error is greater than 30 degrees, use a higher rotation rate
    ...
    if Min>0.3 && abs(angleDifference) > deg2rad(75-60) % When there is a certain distance from the obstacle and the yaw angle is too large
    ...
    [subLiDAR,scan,velmsg,Odom,PubVel,OdomPos,SubOdom,rate,euler, ...
        ...
        end_x,end_y,start,startTime,yaw,yaw_list,x,y,vc_log,wc_log,va_log,wa_log,time_log);
